...
src/lib/findings/         folder owned by findings.ts: the rule's report set
src/lib/findings/index.ts     barrel door: the folder's public surface
src/lib/findings/findings.ts  what `ownership` reports, in report order
src/lib/findings/owners.ts    owners, shells, layers, colocation
src/lib/findings/subject.ts   the linted file, once it is known to be in the model
src/lib/findings/graph-cache.ts one graph per process, revalidated once per lint pass
src/lib/findings/root.ts      resolve a configured root from any working directory
...
src/lib/named-door/       folder owned by named-door.ts: module doors
src/lib/named-door/index.ts   barrel door: the folder's public surface
src/lib/named-door/named-door.ts named-door identity re-export detection
src/lib/named-door/gates.ts entry detection, gate map, crossed-gate lookup
...
"Is this path in the model" is `isInGraphScope` in `src/lib/scope.ts` and nowhere else; it was inline in four places before. The rule-side preamble around it — default the root, `resolveRootDir`, realpath root and file, tolerate either being absent, reject an out-of-scope file — is `resolveSubject` in `src/lib/findings/index.ts` and nowhere else. Use `subject.covers` for a resolved target rather than rebuilding the disjunction.

Two asymmetries inside `Subject` are load-bearing and must survive any tidying. `rootDir` is resolved but **not** realpathed, because it is half the graph cache key. The linted file is scope-tested but not extension-tested again after realpath, so a `.ts` symlink pointing at a `.txt` file is still a subject, whereas resolved targets _are_ extension-tested.

`isInsideDir` has **exactly one** implementation, `src/lib/paths.ts`, used by `named-door/gates.ts`, `findings/owners.ts` and (as `isAtOrInsideDir`) `walk.ts`. It once existed in three copies sharing one defect — appending a separator to a `dir` that already ends in one builds `"//"` and matches nothing, so with `root: "/"` a file plainly inside a directory looked outside it. Fixing one copy left the other two wrong. Do not reintroduce a local copy.
