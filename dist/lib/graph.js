...
function loadPathAliases(rootDir) {
    const configPath = ts.findConfigFile(rootDir, (fileName) => ts.sys.fileExists(fileName));
    ...
    for (const [pattern, targets] of Object.entries(compilerOptions.paths)) {
        ...
        const mappedPrefix = targetStar === -1 ? target : target.slice(0, targetStar);
        aliases.push({
            prefix,
...
function mapAlias(specifier, aliases) {
...
export function resolveSpecifier(specifier, fromDir, aliases = []) {
    let base;
...
function readTsconfigStamp(rootDir) {
    const configPath = ts.findConfigFile(rootDir, (fileName) => ts.sys.fileExists(fileName));
    if (configPath === undefined) {
        return null;
    }
