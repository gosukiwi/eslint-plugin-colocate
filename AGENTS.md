...
Filesystem errors never abort the user's lint run. Missing `root`, linted path not on disk (`--stdin-filename`, processors, deleted mid-run), unreadable file or directory → no findings. All reads go through `src/lib/fs-safe.ts`. One gap remains: a malformed `tsconfig.json` `paths` value can still throw from inside `ts.resolveModuleName`, which is called before the guard in `aliasCandidates` — tracked as an issue.
