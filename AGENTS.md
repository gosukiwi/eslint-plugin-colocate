...
- `import { createRequire as require }` followed by `require("./x")` produces a spurious edge in `parse.ts` to `./x`. That call returns a require function; it doesn't load `./x`.
...
- `parse.ts` takes `require`'s `arguments[0]` at any arity, so `require("./x", opts)` is an edge there and not in `entry`.
- Neither surface sees a **type-position** `import()` inside `parse.ts`'s edge extraction (`ts.isCallExpression` does not match an `ImportTypeNode`). `entry` *does* gate it, as of the `TSImportType` visitor, so this is now a graph-only gap.
