...
Case-insensitive disks: recover import specifiers that differ in case from the on-disk path. Unicode NFC/NFD differences are recovered too, on every platform. Both recoveries happen wherever a resolved path is compared against the graph, which is `canonicalGraphPath` — `entry`, and `collectReExports` in `owners.ts`. The one place still blind to normalization is the graph's own **edge index**: `buildGraphWithConfigs` recovers a resolved target through `filesByLowerCase`, which is keyed on lower case alone, so an edge whose specifier differs from disk only by normalization is still dropped. Config/ESLint **paths** (`root`, linted filename) stay case-sensitive — `root: "SRC"` when the dir is `src` produces no findings.
